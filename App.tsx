...
import { Star, Gavel, FileWarning, ShieldAlert, BadgeAlert, Layers, MessageSquare } from 'lucide-react';
import Header from './components/Header';
import RuleCard from './components/RuleCard';
import { punishmentSystem, moderatorHierarchy, mainRules, topicRules, modRules, complaintRules } from './data';

const App = () => {
  const [activeTab, setActiveTab] = useState<'main' | 'topics'>('main');

  const currentRules = activeTab === 'main' ? mainRules : topicRules;
  ...
  return (
    <div className="min-h-screen bg-slate-950 pb-20">
      ...
      <main className="max-w-4xl mx-auto px-4 pt-6 space-y-8">
        ...
        {/* Navigation Tabs */}
        <div className="flex p-1 bg-slate-900/80 rounded-xl border border-slate-800 backdrop-blur-sm print-break-avoid sticky top-[73px] z-40 shadow-lg shadow-black/20">
          <button
          ...
          >
            ...
            <span className="uppercase tracking-wide">Общие</span>
          </button>
          <button
          ...
          >
            ...
            <span className="uppercase tracking-wide">Топики</span>
          </button>
        </div>
          ...
          {currentRules.map((section) => (
            <section key={section.id} id={`section-${section.id}`} className="scroll-mt-32 animate-fade-in">
              <div className="flex items-center gap-4 mb-6 print-break-avoid">
                 ...
                 <h2 className="text-xl sm:text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-purple-400 uppercase tracking-widest text-center">
                   {section.title}
                 </h2>
                 <div className="h-px bg-slate-800 flex-1"></div>
              </div>
